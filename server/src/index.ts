...
// Get current student's results (authenticated) - MUST come before /:studentId route
app.get('/api/results/student/current', async (req, res) => {
  try {
    ...
    const studentResults = await sequelize.query(`
      SELECT s.id, s.admissionNumber, u.firstName, u.lastName FROM Students s
    ...
    const student = studentResults[0] as any;
    ...
    if (!resultRecords || resultRecords.length === 0) {
      return res.json({ 
        ...
        results: null, 
        subjects: [],
        message: `No results available for ${academicYear} - ${term} term`,
        studentInfo: {
    ...
    const resultRecord = resultRecords[0] as any;
...
// Comments/Complaints Management for Exam Officer and Admin
...
// Student search by admission number - DISABLED (duplicate with mock data)
...
// Results endpoints - DISABLED duplicate endpoint
// app.get('/api/results/student/current', async (req, res) => {
//   try {
...
app.get('/api/payments', async (req, res) => {
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET!) as { userId: number };
    ...
    if (academicYear) {
      whereClause += ' WHERE academicYear = ?';
      replacements.push(academicYear);
    }
    
    if (term) {
      whereClause += whereClause ? ' AND term = ?' : ' WHERE term = ?';
      replacements.push(term);
    }
    ...
    const paymentsQuery = `
      SELECT 
        p.id, p.studentId, p.academicYear, p.term, p.paymentType, p.amount, 
        p.amountPaid, p.balance, p.paymentDate, p.dueDate, p.status, 
        p.paymentMethod, p.receiptNumber, p.description, p.confirmedAt,
        u.firstName || ' ' || u.lastName as studentName,
        cu.firstName || ' ' || cu.lastName as confirmedByName
      FROM Payments p
      LEFT JOIN Users u ON p.studentId = u.id
      LEFT JOIN Users cu ON p.confirmedBy = cu.id
      ${whereClause}
      ORDER BY p.confirmedAt DESC, p.dueDate DESC
    `;
...
// Get financial summary (alternative endpoint)
app.get('/api/payments/summary/financial', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    // Get payment summary for the selected term and academic year
    const [paymentSummary] = await sequelize.query(`
      SELECT 
        SUM(amountPaid) as totalConfirmedPayments,
        COUNT(CASE WHEN status = 'paid' THEN 1 END) as confirmedPaymentsCount,
        COUNT(*) as totalPayments,
    ...
    // Get expenditure summary for the selected term and academic year
    const [expenditureSummary] = await sequelize.query(`
      SELECT 
    ...
    const totalIncome = paymentSummary?.totalIncome || 0;
    const totalConfirmedPayments = paymentSummary?.totalConfirmedPayments || 0;
    const totalExpenditure = expenditureSummary?.totalExpenditure || 0;
    const remainingBalance = totalConfirmedPayments - totalExpenditure;
    ...
    res.json({
      ...
      confirmedPaymentsCount: paymentSummary?.confirmedPaymentsCount || 0,
      totalPayments: paymentSummary?.totalPayments || 0,
      totalExpenditure,
      totalExpenses: expenditureSummary?.totalExpenses || 0,
      remainingBalance,
      netPosition: remainingBalance >= 0 ? 'surplus' : 'deficit'
    });
  } catch (error) {
    console.error('Error fetching financial summary:', error);
    res.status(500).json({ message: 'Server error' });
...
app.get('/api/school-content', (req, res) => {
  try {
    ...
    const contentFilePath = path.join(projectRoot, 'server/data/school-content.json');
    
    console.log('📁 Looking for content file at:', contentFilePath);
...
• Community Impact Recognition 2023`
    ...
    // Try to load saved content from file
    if (fs.existsSync(contentFilePath)) {
      try {
        const savedContent = JSON.parse(fs.readFileSync(contentFilePath, 'utf8'));
        console.log('📖 Loaded saved school content from file');
        res.json(savedContent);
        return;
      } catch (error) {
        console.error('❌ Error reading saved content, using defaults:', error);
      }
    } else {
      console.log('📄 No saved content file found, using defaults');
    }
    
    // Return default content if no saved content exists
    console.log('📄 Using default school content');
    res.json(defaultContent);
    
  } catch (error) {
    console.error('❌ Error serving school content:', error);
...
app.put('/api/school-content', async (req, res) => {
  try {
    ...
    // Save the updated content to a JSON file for persistence
    const fs = require('fs');
    const path = require('path');
    ...
    try {
      ...
      console.log('✅ School content saved to:', contentFilePath);
      
      // Verify the file was created
      if (fs.existsSync(contentFilePath)) {
        const fileSize = fs.statSync(contentFilePath).size;
    ...
    console.log('📝 School content updated and saved to file');
    
    res.json({
      message: 'Content updated successfully',
      content: updatedContent,
      filePath: contentFilePath // Include path in response for debugging
    });
  } catch (error) {
...
app.get('/api/comments', async (req, res) => {
  try {
    const { role, fromRole, studentName } = req.query;
    ...
    if (role === 'admin') {
      ...
      whereClause = 'WHERE fromUser = ? AND fromRole = ?';
      replacements = [studentName, 'student'];
    }
    
    // Add fromRole filter if specified (for admin/exam officer filtering)
    if (fromRole && fromRole !== 'all' && role !== 'student') {
      if (whereClause) {
        whereClause += ' AND fromRole = ?';
      } else {
        whereClause = 'WHERE fromRole = ?';
      }
      replacements.push(fromRole);
    }
    ...
    const commentsQuery = `
      SELECT 
        ...
        term, status, reply, repliedAt, repliedBy, timestamp, createdAt
      FROM Comments
      ${whereClause}
...
app.post('/api/comments/:id/reply', async (req, res) => {
  try {
    const { id } = req.params;
    const { reply, replierRole = 'Admin' } = req.body;
    
    if (!reply || !reply.trim()) {
      return res.status(400).json({ message: 'Reply text is required' });
    }
    
    // Determine replier name based on role
    const replierName = replierRole === 'exam_officer' ? 'Exam Officer' : 'Admin';
    
    // Update comment with reply
    await sequelize.query(`
      UPDATE Comments 
      SET reply = ?, repliedAt = CURRENT_TIMESTAMP, repliedBy = ?, 
          status = 'read', updatedAt = CURRENT_TIMESTAMP
      WHERE id = ?
    `, { replacements: [reply.trim(), replierName, id], type: 'UPDATE' });
...
app.put('/api/admin/users/:id', async (req, res) => {
  try {
    ...
    if (updateData.password && updateData.password.trim() !== '') {
      ...
      userValues.push(updateData.password.trim());
    }
...
app.get('/api/dashboard/student/current', async (req, res) => {
  try {
    ...
    const studentData = studentResults[0] as any;

    // Get fresh attendance data for this specific student
    ...
    `, {
      replacements: [studentData.studentId],
      type: 'SELECT'
    }) as any[];

    const attendance = attendanceResults[0] as any;
    const attendancePercentage = attendance.totalDays > 0 ? 
      Math.round((attendance.presentDays / attendance.totalDays) * 100) : 0;

    // Get recent results for this specific student
    const recentResults = await sequelize.query(`
      SELECT 
        r.academicYear, r.term, r.totalScore, r.averageScore, r.position, r.overallGrade as grade,
        COUNT(sr.id) as subjectCount
      FROM Results r
      LEFT JOIN SubjectResults sr ON r.id = sr.resultId
      WHERE r.studentId = ? AND r.published = 1
      ORDER BY r.academicYear DESC, r.term DESC
      LIMIT 3
    `, {
      replacements: [studentData.studentId],
      type: 'SELECT'
    ...
    res.json({
      student: {
        ...
        house: studentData.house || 'Blue House', // Default house for new students
        ...
        class: {
          ...
          name: studentData.className || 'JSS 1A', // Default class for new students
          level: studentData.classLevel || 'JSS 1'
        }
      },
      attendance: {
...
// Get current student's results - MOVED TO EARLIER POSITION TO AVOID ROUTE CONFLICT
...
app.get('/api/dashboard/accountant/stats', async (req, res) => {
  try {
    ...
    res.json({
      // Flat structure that matches frontend expectations
      ...
      totalExpenditures: expenditureStats.totalExpenses || 0,
      profitMargin: totalIncome > 0 ? ((netBalance / totalIncome) * 100).toFixed(2) : 0,
      
      // Keep nested structure for detailed breakdowns
      payments: {
...
app.post('/api/accountant/payments/confirm', async (req, res) => {
  try {
    ...
    // Get financial summary for the selected term and academic year
    const [paymentSummary] = await sequelize.query(`
      SELECT 
        SUM(amountPaid) as totalConfirmedPayments,
        COUNT(CASE WHEN status = 'paid' THEN 1 END) as confirmedPaymentsCount,
    ...
    const totalIncome = paymentSummary?.totalConfirmedPayments || 0;
    const totalExpenditure = expenditureSummary?.totalExpenditure || 0;
    ...
    res.json({
      ...
      financialSummary: {
        academicYear,
        term,
        totalConfirmedPayments: totalIncome,
        confirmedPaymentsCount: paymentSummary?.confirmedPaymentsCount || 0,
        totalPayments: paymentSummary?.totalPayments || 0,
...
app.get('/api/accountant/financial-summary', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
...
app.put('/api/admin/users/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { 
      firstName, lastName, email, role, office, phone, isActive, password,
      employeeId, admissionNumber, classId, assignedOffice 
    } = req.body;
    ...
    let updateQuery = `
      UPDATE Users 
      SET firstName = ?, lastName = ?, email = ?, role = ?, office = ?, phone = ?, isActive = ?`;
    let replacements = [firstName, lastName, email, role, office, phone, isActive];

    // Add password to update if provided
    if (password && password.trim() !== '') {
      updateQuery += `, password = ?`;
      replacements.push(password.trim());
    }
...
app.get('/api/teacher/messages', async (req, res) => {
  try {
    ...
    // Get teacher's user information
    const [teacher] = await sequelize.query(`
      SELECT u.firstName, u.lastName, u.role FROM Users u WHERE u.id = ? AND u.role = 'teacher'
    `, {
      replacements: [decoded.userId],
      type: 'SELECT'
    }) as any[];
    ...
    const messages = await sequelize.query(`
      SELECT 
        id, subject, message, toRole as recipient, status, createdAt,
        reply, repliedAt, repliedBy, priority
      FROM Comments
      WHERE fromUser = ? AND fromRole = 'teacher'
...
app.post('/api/teacher/messages', async (req, res) => {
  try {
    ...
    // Insert message into Comments table
    const [messageResult] = await sequelize.query(`
      INSERT INTO Comments (
        fromUser, fromRole, toUser, toRole, subject, message, priority, status
      ) VALUES (?, 'teacher', ?, ?, ?, ?, ?, 'unread')
    `, {
      replacements: [
        teacherName,
        recipient === 'admin' ? 'Admin' : 'Exam Officer',
        recipient,
        subject,
        message,
        priority === 'medium' ? 'normal' : priority || 'normal'
      ],
      type: 'INSERT'
    }) as any[];
    ...
    const [newMessage] = await sequelize.query(`
      SELECT 
        id, subject, message, toRole as recipient, status, createdAt, priority
      FROM Comments
      WHERE id = ?
    `, {
      replacements: [messageResult],
      type: 'SELECT'
    }) as any[];

    console.log('📧 Teacher message sent:', {
      from: teacherName,
      to: recipient,
      subject: subject,
      id: newMessage.id
...
app.get('/api/dashboard/teacher/stats', async (req, res) => {
  try {
    ...
    const [totalMessages] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments WHERE fromUser = ? AND fromRole = 'teacher'
    `, {
      replacements: [teacherName],
      type: 'SELECT'
    }) as any[];

    const [pendingReplies] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'teacher' AND status IN ('unread', 'read') AND reply IS NULL
    `, {
      replacements: [teacherName],
      type: 'SELECT'
    }) as any[];

    const [resolvedComplaints] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'teacher' AND reply IS NOT NULL
    `, {
      replacements: [teacherName],
      type: 'SELECT'
    }) as any[];
    ...
    const [unreadMessages] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'teacher' AND status = 'unread'
    `, {
      replacements: [teacherName],
      type: 'SELECT'
    }) as any[];
  ...
  try {
    ...
    const [teacher] = await sequelize.query(`
      SELECT 
        ...
        s.dateOfBirth, s.address
      FROM Users u
      LEFT JOIN Staff s ON u.id = s.userId
      WHERE u.id = ? AND u.role = 'teacher' AND u.isActive = 1
    `, {
      replacements: [decoded.userId],
      type: 'SELECT'
    }) as any[];
...
// Get exam officer messages
app.get('/api/exam-officer/messages', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    // Get exam officer's user information
    const [examOfficer] = await sequelize.query(`
      SELECT u.firstName, u.lastName, u.role FROM Users u WHERE u.id = ? AND u.role = 'exam_officer'
    `, {
      replacements: [decoded.userId],
      type: 'SELECT'
    }) as any[];
    ...
    const examOfficerName = `${examOfficer.firstName} ${examOfficer.lastName}`;

    // Get messages sent TO this exam officer
    const incomingMessages = await sequelize.query(`
      SELECT 
        id, fromUser, fromRole, subject, message, toRole as recipient, status, createdAt,
        reply, repliedAt, repliedBy, priority
      FROM Comments
      WHERE toRole = 'exam_officer'
      ORDER BY createdAt DESC
    ...
    // Get messages sent FROM this exam officer
    const outgoingMessages = await sequelize.query(`
      SELECT 
        id, subject, message, toRole as recipient, status, createdAt,
        reply, repliedAt, repliedBy, priority
      FROM Comments
      WHERE fromUser = ? AND fromRole = 'exam_officer'
      ORDER BY createdAt DESC
    `, {
      replacements: [examOfficerName],
      type: 'SELECT'
    }) as any[];

    const formattedIncoming = incomingMessages.map((msg: any) => ({
      id: msg.id,
      type: 'incoming',
    ...
    const formattedOutgoing = outgoingMessages.map((msg: any) => ({
      id: msg.id,
      type: 'outgoing',
      subject: msg.subject,
      message: msg.message,
    ...
    console.log(`📧 Retrieved ${formattedIncoming.length} incoming and ${formattedOutgoing.length} outgoing messages for exam officer: ${examOfficerName}`);

    res.json({
      incomingMessages: formattedIncoming,
      outgoingMessages: formattedOutgoing,
      totalIncoming: formattedIncoming.length,
      totalOutgoing: formattedOutgoing.length
    });
  } catch (error) {
    console.error('Error fetching exam officer messages:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Send exam officer message
app.post('/api/exam-officer/messages', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    const [messageResult] = await sequelize.query(`
      INSERT INTO Comments (
        fromUser, fromRole, toUser, toRole, subject, message, priority, status, createdAt
      ) VALUES (?, 'exam_officer', ?, ?, ?, ?, ?, 'unread', datetime('now'))
    `, {
      replacements: [
        examOfficerName,
        recipient === 'admin' ? 'Admin' : recipient,
        recipient,
        subject,
        message,
        priority === 'medium' ? 'normal' : priority || 'normal'
    ...
    console.log('📧 Exam officer message sent:', {
      from: examOfficerName,
      to: recipient,
      subject: subject,
      id: newMessage.id
    });
    ...
    console.error('Error sending exam officer message:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Reply to message as exam officer
app.patch('/api/exam-officer/messages/:id/reply', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    await sequelize.query(`
      UPDATE Comments 
      SET reply = ?, status = 'read', repliedAt = datetime('now'), repliedBy = ?, updatedAt = datetime('now')
      WHERE id = ? AND toRole = 'exam_officer'
    `, {
      replacements: [reply, examOfficerName, id],
      type: 'UPDATE'
    });

    console.log('📧 Exam officer replied to message:', {
      messageId: id,
      repliedBy: examOfficerName
    });
...
// Get exam officer dashboard stats
app.get('/api/dashboard/exam-officer/stats', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    // Get message statistics
    const [totalIncoming] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments WHERE toRole = 'exam_officer'
    `, {
      type: 'SELECT'
    }) as any[];

    const [totalOutgoing] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments WHERE fromUser = ? AND fromRole = 'exam_officer'
    `, {
      replacements: [examOfficerName],
      type: 'SELECT'
    }) as any[];
    ...
    const [pendingReplies] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE toRole = 'exam_officer' AND status IN ('unread', 'read') AND reply IS NULL
    `, {
      type: 'SELECT'
    }) as any[];

    const [repliedMessages] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE toRole = 'exam_officer' AND reply IS NOT NULL
    `, {
      type: 'SELECT'
    }) as any[];

    res.json({
      totalIncoming: totalIncoming.count || 0,
      totalOutgoing: totalOutgoing.count || 0,
      unreadMessages: unreadMessages.count || 0,
      pendingReplies: pendingReplies.count || 0,
      repliedMessages: repliedMessages.count || 0
    });
  } catch (error) {
    console.error('Error fetching exam officer stats:', error);
    res.status(500).json({ message: 'Server error' });
...
app.get('/api/dashboard/exam-officer/current', async (req, res) => {
  try {
    ...
    // Get exam officer's complete information
    const [examOfficer] = await sequelize.query(`
      SELECT 
        u.id, u.firstName, u.lastName, u.email, u.phone, u.role, u.office,
        s.employeeId, s.qualification, s.experience, s.subjects, s.salary,
    ...
    res.json({
      examOfficer: {
        ...
        subjects: examOfficer.subjects,
        salary: examOfficer.salary,
        dateOfBirth: examOfficer.dateOfBirth,
